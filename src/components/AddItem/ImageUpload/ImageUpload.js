...
const imageUpload = props => {
	...
	const validateImage = file => {
		...
		if (!validExtensions.includes(fileExtension)) {
			arr.push(
				`only these extensions are allowed (${validExtensions.join(', ')})`
			);
		}
	...
	const clearImageUploads = id => {
		// if (props.isInEditingMode && props.url) {
		// 	props.deleteImageByUrl(props.url);
		// }
	...
	return (
		<div className={classes['image-upload']}>
			<input
				...
				onChange={e => uploadImageHandler(e, props.id)}
			/>
			{props.url ? (
				<div
				...
				>
					<div className={classes['image-preview__overlay']}>
						<a
							onClick={() => clearImageUploads(props.id)}
							className={classes['overlay__links']}
						>
							Cancel
						</a>
						<label htmlFor={props.id} className={classes['overlay__links']}>
							Change
						</label>
					</div>
				</div>
				...
				<>
					<label htmlFor={props.id} className={classes['image-upload__icon']}>
