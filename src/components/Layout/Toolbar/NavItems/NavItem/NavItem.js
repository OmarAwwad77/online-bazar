...
const NavItem = (props) => {
    ...
    const createUnorderedList = (arr) => {
        return <ul className={classes.nested_dropdown}>
            {arr.map((el) => (
                <li key={el} ><NavLink smooth to="/#products">{el}</NavLink></li>
            ))}
        </ul>
    }
    ...
    return (
        <li className={classes.nav_item}>
            <NavLink smooth exact={props.exact ? true : false} activeClassName={props.noActiveStyle ? null : classes.active} to={props.path}>{props.children}</NavLink>
